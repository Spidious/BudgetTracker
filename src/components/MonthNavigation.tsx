...
export default function MonthNavigation() {
  const { currentMonthId, navigateMonth } = useBudgetStore();
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <div style={{ textAlign: 'center', minWidth: 180 }}>
        <div style={{ fontSize: 20, fontWeight: 600, color: '#e2e8f0' }}>
          {formatMonthLabel(currentMonthId)}
        </div>
        {isCurrentMonth && (
          <div style={{ fontSize: 11, color: '#10b981', marginTop: 2 }}>Current Month</div>
        )}
        {isHistorical && (
          <div style={{ fontSize: 11, color: '#8892b0', marginTop: 2 }}>Historical</div>
        )}
        {!isCurrentMonth && !isHistorical && (
          <div style={{ fontSize: 11, color: '#6366f1', marginTop: 2 }}>Future</div>
        )}
      </div>
